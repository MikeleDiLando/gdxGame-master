...
public class Bird extends Entity {
    ...
    public Bird(Vector3 pos, Box2DWorld box2d, EnityState state){
        super();
        maxHeight = setHeight();
    ...
    private void setNewState(float delta) {
        if(coolDown > 0){
            ...
            if(MathUtils.randomBoolean(.2f)){
                state = EnityState.FLYING;
            } else if(MathUtils.randomBoolean(.5f)) {
                state = EnityState.FEEDING;
                coolDown = .5f;
            } else if(MathUtils.randomBoolean(.3f)) {
                state = EnityState.WALKING;
                coolDown = 1f;
            }
        }
    }
    ...
    private boolean isFlying() {
        return state == EnityState.FLYING;
    }

    private boolean isHovering(){
        return state == EnityState.HOVERING;
    }

    private boolean isLanding(){
        return state == EnityState.LANDING;
    }

    private boolean isFeeding(){
        return state == EnityState.FEEDING;
    }

    private boolean isWalking(){
        return state == EnityState.WALKING;
    }
